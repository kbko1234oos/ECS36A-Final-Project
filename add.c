...
void add_to_canvas(canvas* old_canvas, int add_index, bool add_row) {
    ...
    if (add_row) {
        ...
        makeCanvas(new_canvas, blank_char);
    } else {
    ...
    if (add_row) {
        ...
        if (i < add_index) {
          copy_row(old_canvas, new_canvas, (new_canvas->numRows), i, i);
        } else {
          copy_row(old_canvas, new_canvas, (new_canvas->numRows), i, i + 1);
        }
      }
    } else {
        for (int i = 0; i < new_canvas->numCols; ++i) {
        if (i < add_index) {
          copy_column(old_canvas, new_canvas, (new_canvas->numCols), i, i);
        } else {
          copy_column(old_canvas, new_canvas, (new_canvas->numCols), i, i + 1);
        }
      }
    }
}
